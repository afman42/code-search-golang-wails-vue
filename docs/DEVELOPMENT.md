...
- **Composables**: Extract reusable state and logic into composables under `frontend/src/composables/`. Composables should encapsulate a single responsibility (e.g., `useSearch` for search state, `useLogStreaming` for log streaming, `useEditorDetection` for editor detection) and expose a clean return interface. Lifecycle hooks (`onMounted`, `onUnmounted`) belong in composables, not components, so the logic is testable independently of the template.
