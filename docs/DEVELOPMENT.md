...
- **Toolchain**: `golangci-lint` v2 and `staticcheck` require Go ≥ 1.26, so `go install` switches toolchains automatically (`go.mod` pins `go 1.25.0`; CI's `setup-go` uses 1.26.6). Install the versions CI pins rather than `@latest`, or local results and CI can disagree: `go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.13.2`, `go install honnef.co/go/tools/cmd/staticcheck@v0.7.0`, `go install golang.org/x/vuln/cmd/govulncheck@v1.1.4`.
...
- **CI**: `.github/workflows/build.yml` runs gofmt check → `go vet` → `golangci-lint` → `staticcheck` → `govulncheck` before the Go tests, so a lint regression fails the `test` job and blocks the `build` job. The installed tools are pinned — `golangci-lint` `v2.13.2` (via `golangci/golangci-lint-action@v9`), `staticcheck@v0.7.0`, `govulncheck@v1.1.4`, and in the build job `wails@v2.14.0` (tracking `github.com/wailsapp/wails/v2` in `go.mod`; a CLI newer than the library is a real breakage source). These were `@latest`, which made builds non-reproducible: an upstream release could red CI with no repo change, and two runs of the same commit could disagree. One tool is still unpinned: `garble@latest` in the build job, so that job can still be reddened by an upstream release. After the tests, CI enforces the 80% coverage gate and uploads `coverage.out` as a `go-coverage` artifact.
- **CI is Linux-only — platform files are unverified in CI**: the `test` job runs on `ubuntu-latest` with no matrix, so the build-tagged platform files — `app.go` (`//go:build linux`), `appDarwin.go` (`//go:build darwin`), `appWindows.go` (`//go:build windows`) — are only compiled for Linux there. `appWindows.go` gets a second look from the `build` job, which cross-compiles a Windows binary from Linux (mingw gcc); `appDarwin.go` is never built in CI. Editing a platform file is therefore not verified by CI, so cross-check locally with `GOOS=darwin go build ./...` / `GOOS=windows go build ./...` before pushing. The embed target is not an issue in CI because the test job builds the real `frontend/dist` first (`Build Frontend Dist`); there is no stub-embed step and no matrix to gate it.
- **Triggers and release**: the workflow triggers only on a push to `main` — there is no `pull_request` trigger and no `v*` tag trigger. There are two jobs (`test`, `build`); the `build` job uploads `code-search-golang-linux` and `code-search-golang-windows` artifacts but there is no `release` job, so publishing a release is a manual step.
