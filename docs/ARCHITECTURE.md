...
| `text_extensions.go`     | Set of 173 known-text extensions (.go, .ts, .py, .md, .vue, .toml, .txt, etc.) that skip the binary detection probe entirely. Exposes `GetKnownTextExtensions()` — a Wails binding the frontend loads into search state. See [`EXTENSIONS.md`](EXTENSIONS.md). |
| `editors.go`             | Editor detection (25 editors), the `editorCatalog` table, `catalogEntry`, `countEditorsFromSnapshot`, and the sole `OpenInEditorByName` launch dispatcher plus `getJetBrainsEditor` file-extension routing. |
...
- **Known-text set** (`text_extensions.go` → `knownTextExtensions`) — 173 extensions that skip the binary probe. The single source of truth for "is this file text?"
...
- **Editor detection**: probes 25 editor commands in parallel via `exec.LookPath` (`editors.go`). Detected editors include VS Code, VSCodium, Sublime, Geany, JetBrains IDEs (GoLand, PyCharm, IntelliJ, WebStorm, PhpStorm, CLion, Rider — routed by file extension), Android Studio, Emacs, Neovim, Neovide, Vim, Code::Blocks, Dev-C++, Notepad++, Visual Studio, Eclipse, NetBeans.
...
- **Known-text extension shortcut**: 173 text extensions skip the binary probe entirely — no `open`/`read`/`close` syscall per known-text file. The set is exposed to the frontend via the `GetKnownTextExtensions()` binding and loaded into search state.
...
- **CI** (`.github/workflows/build.yml`) — two jobs (`test`, then `build` with `needs: test`), triggered only by a push to `main`. The test job runs on `ubuntu-latest` only, so it never compiles the build-tagged platform files: `appDarwin.go` (`//go:build darwin`) and `appWindows.go` (`//go:build windows`) are invisible to a Linux-only build. Go tests run `-race -covermode=atomic -coverprofile=coverage.out`, followed by the 80% coverage gate; lint, frontend, E2E, and `vue-tsc` run in the same straight-line sequence. The `build` job cross-compiles a Linux binary and a Windows binary (mingw gcc) from Linux — that cross-compile is the only CI compile-time check `appWindows.go` gets, and `appDarwin.go` is not built in CI at all. There is no OS matrix, no `pull_request` trigger, and no release job: artifacts are uploaded, and releases are cut manually.
